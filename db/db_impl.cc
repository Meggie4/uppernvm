  ...
  for (size_t i = 0; i < filenames.size(); i++) {
    if (ParseFileName(filenames[i], &number, &type)) {
      ...
      if (!keep) {
        ...
        //////////meggie
        if(type == kMapFile){
            MemTable* nvmtbl = versions_.memtable_mappings_[number];
            versions_.memtable_mappings_.erase(number);
            nvmtbl->Unref();
        }
        //////////meggie
        /////////////////meggie
...
///////////////////meggie
MemTable* DBImpl::CreateNVMTable(std::string new_map_number, 
        size_t size){
...
void DBImpl::MovetoNVMTable(Iterator* iter, &meta){
    iter->SeekToFirst();
    size_t count = 0;
    bool has_current_user_key = false;
    std::string current_user_key;
    SequenceNumber last_sequence_for_key = kMaxSequenceNumber;
    memtable* nvmtbl = CreateNVMTable(meta->number, 
    ...
    for (; iter->Valid(); iter->Next()) {
      ...
      if(!drop){
         nvmtbl->Add(iter->GetNodeKey());
         meta->largest.DecodeFrom(key);
         //count++;
      }
    }
    nvmtbl->finish();
    meta->file_size = nvmtbl->file_size();
    versions_.memtable_mappings.insert(std::make_pair(meta->number, nvmtbl));
}
///////////////////meggie
...
Status DBImpl::OpenCompactionOutputFile(CompactionState* compact) {
  ...
  //////////////meggie
  if(compact->isNVMMemtable){
      MemTable* nvmtbl = CreateNVMTable(file_number, 
              compact->compaction->MaxOutputFileSize());
      compact->nvmtbl = nvmtbl;
      versions_.memtable_mappings_[file_number] = nvmtbl;
      return Status::OK();
  }
  else 
  //////////////meggie
...
Status DBImpl::FinishCompactionOutputFile(CompactionState* compact,
  ...
  assert(compact != nullptr);
  ////////////meggie
  assert(compact->outfile != nullptr || compact->isNVMMemtable);
  assert(compact->builder != nullptr || compact->nvmtbl);
  ////////////meggie
  ...
  /////////////meggie
  if(compact->isNVMMemtable){
      compact->nvmtbl->Finish();
      const uint64_t current_bytes = compact->nvmtbl->FileSize();
      compact->current_output()->file_size = current_bytes;
      compact->total_bytes += current_bytes;
      compact->nvmtbl = nullptr;
      return Status:OK();
  }
  /////////////meggie
...
Status DBImpl::DoCompactionWork(CompactionState* compact) {
  ...
  for (; input->Valid() && !shutting_down_.Acquire_Load(); ) {
    ...
    if (!drop) {
      ...
      if(!compact->isNVMMemtable){
          ...
          if (compact->nvmtbl == nullptr) {
            status = OpenCompactionOutputFile(compact);
            if (!status.ok()) {
              break;
            }
          }
          if (compact->nvmtbl->NumEntries() == 0) {
            compact->current_output()->smallest.DecodeFrom(key);
          }
          compact->current_output()->largest.DecodeFrom(key);
          compact->nvmtbl->Add(key, input->value());

          // Close output file if it is big enough
          if (compact->nvmtbl->FileSize() >=
              compact->compaction->MaxOutputFileSize()) {
            status = FinishCompactionOutputFile(compact, input);
            if (!status.ok()) {
              break;
  ...
  if(!compact->isNVMMemtable){
    ...
    if (status.ok() && compact->nvmtbl != nullptr) {
        status = FinishCompactionOutputFile(compact, input);
    }
  }
  ///////////meggie
