...
//////////////meggie
static Iterator* GetNVMTableIterator(void* arg, 
        const ReadOptions& options, 
        const Slice& file_value) {
    std::map<uint64_t, MemTable*>& memtable_mappings = 
        *(reinterpret_cast<std::map<uint64_t, MemTable*>*>(arg));
    return memtable_mappings[DecodeFixed64(file_value.data())]->NewIterator(); 
}
//////////////meggie
  ...
  for (int which = 0; which < 2; which++) {
    if (!c->inputs_[which].empty()) {
      if (c->level() + which == 0) {
        ...
        ///////////meggie
        for (size_t i = 0; i < files.size(); i++) {
            list[num++] = memtable_mappings_[files[i]->number]->NewIterator(); 
        }
        ///////////meggie
      } else {
        ...
        ////////////meggie
            if(c->level() + which >= 3){
                ...
                list[num++] = NewTwoLevelIterator(
                    new Version::LevelFileNumIterator(icmp_, &c->inputs_[which]),
                    &GetNVMTableIterator, memtable_mappings_, options); 
            }   
        }
        ////////////meggie
    }
